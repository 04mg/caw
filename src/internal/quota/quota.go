...
func Register(mux *http.ServeMux, store *state.Store) {
	...
	mux.HandleFunc("GET /api/quotas/settings", func(w http.ResponseWriter, r *http.Request) {
		settings, err := store.GetQuotaSettings()
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
	...
	mux.HandleFunc("PUT /api/quotas/settings", func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("POST /api/quotas/copilot/device-codes", func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("GET /api/quotas/copilot/device-codes/{device_code}", func(w http.ResponseWriter, r *http.Request) {
		deviceCode := r.PathValue("device_code")
		if deviceCode == "" {
			http.Error(w, "device_code required", http.StatusBadRequest)
			return
		}
		tr, err := pollDeviceToken(deviceCode)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
